...
Text Processing: Code was inspired by (and heavily modified from) code written elsewhere. Specific references are given in the Jupyter Notebook file in which most of the processing and modeling was conducted (`nltk_processing.ipynb`).
...
In `nltk_processing.ipynb` lives the code used to clean and aggregate the data scraped in the above step into a manageable format. Further, it contains code 
