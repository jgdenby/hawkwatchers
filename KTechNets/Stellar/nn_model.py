...
def predict(model, new_text):
    ...
    releaserates_df = pd.read_csv(
        "hawksite/hawk_tracker/allreleaserates.csv", index_col=0
    )

    # computes tf-idf for all release texts and creates a vector representation
    # of each document's scores
    TFVectorizer = sklearn.feature_extraction.text.TfidfVectorizer(
        max_df=15, stop_words="english", norm="l2"
    )
    ...
    if model == "NN":
        ...
        clf = sklearn.ensemble.BaggingClassifier(
            tree, n_estimators=100, max_samples=0.8
        )
